...
const AGENTS_ROOT = process.env.AGENTS_ROOT || path.dirname(fileURLToPath(import.meta.url));
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "call_agent_command") {
      ...
      let finalPrompt = await resolveProbes(prompt);

      // Auto-inject ALL files from {agent}/knowledge/ not already catted
      // This ensures retrospective-written lessons are always included
      const agentKnowledgeDir = path.join(AGENTS_ROOT, agent, "knowledge");
      try {
        const knowledgeFiles = await fs.readdir(agentKnowledgeDir);
        let autoKnowledge = "";
        for (const file of knowledgeFiles.filter(f => f.endsWith(".md"))) {
          if (!cattedBasenames.has(file)) {
            const content = await fs.readFile(path.join(agentKnowledgeDir, file), "utf-8").catch(() => "");
            if (content) autoKnowledge += `\n\n## [Knowledge] ${file.replace(".md","").replace(/-/g," ")}\n${content}`;
          }
        }
        if (autoKnowledge) finalPrompt += `\n\n# Agent Knowledge Base${autoKnowledge}`;
      } catch (_) { /* knowledge dir may not exist */ }
