...
4. **Draft Plan:** Create a detailed Implementation Plan and **write it to `[FEATURE]_IMPLEMENTATION_PLAN.md`** immediately.
   ...
   - **Context7 Dependency Validation**: If the plan involves external libraries, frameworks, SDKs, cloud services, or modern APIs, query the `context7` MCP server before finalising the ADR or architecture plan:
     - Call `resolve-library-id` with `libraryName` and a short `query` describing the architectural decision
     - Call `query-docs` with the resolved `libraryId` and a focused `query` for the architectural topic
     - Use the evidence to validate: capability claims, integration constraints, API maturity, version-specific behaviour, and migration risks
     - Summarise findings in `architect.md` under `## Dependency Evidence (Context7)` and cite them in `ADR.md` under `## Evidence` when they affect the decision
     - Do NOT embed large code blocks -- that is implementation-plan material for backend/frontend agents
5. **Validation & Persistence:** Inform the user the plan is ready at `[FEATURE]_IMPLEMENTATION_PLAN.md` and request approval.
   - **AUTOMATED PLAN VERIFICATION:** Prior to halting for approval, you must verify that the plan contains the `## Dependency Evidence (Context7)` section when external libraries are involved. If missing or empty, **HALT** and regenerate with proper Context7 references.
